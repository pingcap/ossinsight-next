...
WITH RECURSIVE seq(idx, current_period_day, past_period_day) AS (
    ...
    FROM (
        SELECT
            ...
            {% endcase %} AS day,
            COUNT(DISTINCT number) AS reviewed_prs
...
SELECT
    ...
    IFNULL(cp.reviewed_prs, 0) AS current_period_day_total,
    s.past_period_day AS past_period_day,
    IFNULL(pp.reviewed_prs, 0) AS past_period_day_total
FROM seq s
