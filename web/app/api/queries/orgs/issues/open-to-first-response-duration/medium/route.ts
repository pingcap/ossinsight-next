...
WITH repos AS (
    ...
    SELECT
        ...
        PERCENT_RANK() OVER (PARTITION BY iwo.period ORDER BY (iwr.first_responsed_at - iwo.opened_at)) AS percentile
    FROM issues_with_opened_at iwo
    JOIN issues_with_first_responsed_at iwr USING (repo_id, number)
    WHERE
