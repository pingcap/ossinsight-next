...
export function AnalyzeSelector ({ tuple, onTupleChange }: AnalyzeSelectorProps) {
  ...
  return (
    <div className="TextInput flex items-center divide-x">
      ...
      {tuple.type === 'user' && <GHUserSelector user={tuple.value} onUserSelected={handleUserSelected} renderInput={renderInput} />}
      {tuple.type === 'repo' && <GHRepoSelector repo={tuple.value} onRepoSelected={handleRepoSelected} renderInput={renderInput} />}
    </div>
  );
}
